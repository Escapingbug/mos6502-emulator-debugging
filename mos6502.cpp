...
void mos6502::DebugCommandline(uint16_t src) {
  std::cout << "Break at " << std::hex << src << std::endl;
  ...
  while (1) {
	...
	if (args.size() < 1) {
	  std::cout << "Incorrect commands" << std::endl;
	  continue;
	}
	...
	auto iter = commands.find(args[0][0]);
...
void mos6502::DumpStatus(std::vector<std::string> &args) {
...
/// usage: x [MEM] [LEN]
void mos6502::DumpMem(std::vector<std::string> &args) {
  if (args.size() < 3) {
	std::cout << "usage: x [MEM] [LEN]" << std::endl;
	return;
  }
...
void mos6502::AddBreakpoint(std::vector<std::string> &args) {
  if (args.size() < 2) {
	std::cout << "usage: b [MEM]" << std::endl;
	return;
  }
  ...
  std::cout << "Breakpoint #" << bps.size() << std::endl;
}

void mos6502::RemoveBreakpoint(std::vector<std::string> &args) {
  if (args.size() < 2) {
	std::cout << "usage: d [NUM]" << std::endl;
	return;
  }
...
void mos6502::MemWatch(std::vector<std::string> &args) {
  if (args.size() < 2) {
	std::cout << "usage: w [ADDR]" << std::endl;
	return;
  }
  ...
  watches.push_back(addr);

  std::cout << "Watch on #" << watches.size() << std::endl;
}

void mos6502::RemoveMemWatch(std::vector<std::string> &args) {
  if (args.size() < 2) {
	std::cout << "usage: r [NUM]" << std::endl;
	return;
  }
  ...
  std::cout << "Removed watch #" << num << std::endl;
}

uint8_t mos6502::Read(uint64_t where) {
  uint8_t byte = InternalRead(where);
  for (auto addr : watches) {
	if (addr == DebugPC) {
	  printf("0x%x: read mem[0x%llx] -> 0x%x\n", DebugPC, where, byte);
	}
  }
  return byte;
}

void mos6502::Write(uint64_t where, uint64_t what) {
  for (auto addr : watches) {
	if (addr == DebugPC) {
	  printf("0x%x: write mem[0x%llx] <- 0x%llx\n", DebugPC, where, what);
	}
  }
  InternalWrite(where, what);
}
...
mos6502::mos6502(BusRead r, BusWrite w)
{
	...
	// init debug comamnds
	commands.insert(std::pair<char, DebugCommand>('b', &mos6502::AddBreakpoint));
	commands.insert(std::pair<char, DebugCommand>('d', &mos6502::RemoveBreakpoint));
	commands.insert(std::pair<char, DebugCommand>('i', &mos6502::DumpStatus));
	commands.insert(std::pair<char, DebugCommand>('x', &mos6502::DumpMem));
	commands.insert(std::pair<char, DebugCommand>('w', &mos6502::MemWatch));
	commands.insert(std::pair<char, DebugCommand>('r', &mos6502::RemoveMemWatch));

	
	InternalWrite = (BusWrite)w;
	InternalRead = (BusRead)r;
	Instr instr;
...
void mos6502::Run(uint32_t n)
{
	...
	while(start + 1000000 > cycles && !illegalOpcode)
	{
...
{
    printf("Illegal Opcode encountered at pc: 0x%x\n", pc);
	illegalOpcode = true;
}
