...
private:
	...
	BusRead InternalRead;
	BusWrite InternalWrite;
	...
	uint16_t DebugPC = 0;
	bool DebuggingFirst = false;
	std::vector<uint16_t> bps;
	std::vector<uint16_t> watches;

	void MemWatch(std::vector<std::string> &args);
	void RemoveMemWatch(std::vector<std::string> &args);
	void DumpStatus(std::vector<std::string> &args);
	void DumpMem(std::vector<std::string> &args);
	void AddBreakpoint(std::vector<std::string> &args);
	void RemoveBreakpoint(std::vector<std::string> &args);

	typedef void (mos6502::*DebugCommand)(std::vector<std::string> &args);
	std::map<char, DebugCommand> commands;

	void Write(uint64_t where, uint64_t what);
	uint8_t Read(uint64_t where);
...
public:
	...
	void DebugCommandline(uint16_t src);
};
